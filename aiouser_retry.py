...
class AioInfoSpider:
    ...
    def start(self):
        ...
        while start < 300000000:
            if self.r.scard('vproxies') < 50:
                print('代理池数量小于50，休息15秒')
                time.sleep(15)
                continue
            count = self.r.llen('bilibili0')
            mids = self.r.lrange('bilibili0', start, end)
            #print('当前进度为%d-%d'%(start, end))
            #print(self.err)
            self.run(mids)
            print('成功数量：%d' % self.count)
            print('当前数量: %d' % (count-self.r.llen('bilibili0')))
            
            
    async def http_get(self, mid, proxy):
        info_url = 'http://api.bilibili.com/x/space/acc/info?mid=%s&jsonp=jsonp' % mid
        stat_url = 'http://api.bilibili.com/x/relation/stat?vmid=%s&jsonp=jsonp' % mid
        upstat_url = 'http://api.bilibili.com/x/space/upstat?mid=%s&jsonp=jsonp' % mid
        headers = {
            ...
            async with aiohttp.ClientSession(headers=headers) as session:
                ...
                async with session.get(info_url, timeout=15, proxy=proxy) as resp:
                    if resp.status == 200:
                        ...
                        if info:
                            if info.get('code'):
                                self.r.lrem('bilibili0', 0, str(mid))
                                return None
                            info = info.get('data')
                            result.update({
                ...
                self.count += 1
        # 这些异常都是代理不能正常使用的异常        
        except (aiohttp.ClientError, asyncio.TimeoutError, aiohttp.client_exceptions.ClientConnectorError) as e:
            self.r.rpush(self.invset, str(mid))
            self.r.srem(self.vset, proxy)
            #print(str(e))
        # 属性或者键错误的异常    
