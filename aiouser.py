...
class AioInfoSpider:
    def __init__(self):
        ...
        self.mset = self.mongo['bilibili']['user'] # 存储数据的mongo集合
        self.r = redis.Redis(decode_responses=True)
        self.invset = 'invUser' # 存储未被抓取的Redis列表
        self.inv404 = 'inv404' # 存储404状态码的用户
    ...
    def start(self):
        ...
        '''
        once = 150 # 每次运行的协程量，根据你的有效代理数量判断.但值最好不要超过1000(asyncio的限制)，一般为代理*1, 保守可以*0.8
                   # 当然你也可以过度消费代理，代理 * (5-10)。不过这样会导致代理在短时间内被封, 针对于时效较短的代理
        start = 0  
        end = start + once
        self.vset = 'vproxies'
